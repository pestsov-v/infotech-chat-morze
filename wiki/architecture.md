...
Файл `app.ts` является точкой входа в приложение. В нём инициализируются классы: 
...
- инициализация `http` и `https` серверов.
...
- маршрута по-короткому можно получить страницу документации. 
...
- `api.model.enum.ts` - унификатор названий моделей сущностей.
- `api.router.path.ts` - API пути маршрутизации доступа.
- `api.router.ts` - маршрутизатор, в который подключается маршрутизаторы различных сущностей API с последующим присвоением ему соответствующего пути.
